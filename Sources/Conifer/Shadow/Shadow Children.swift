...
struct ShadowChildren<Parent : Shadow, Child/* : Shadow */> : AsyncSequence {	// TODO: Constrain `Child` to `Shadow` when `Shadow` conforms to `any Shadow`.
	...
	struct AsyncIterator : AsyncIteratorProtocol {
		...
		private init(graph: ShadowGraph, parentLocation: ShadowGraph.Location) {
			self.graph = graph
		...
		private enum State {
			...
			case initial(parentLocation: ShadowGraph.Location)
			
			/// The iterator is iterating through the parent component's children.
			///
			/// - Parameter childLocations: A list of absolute locations of children of the parent component that are to be returned or traversed.
			case shallow(childLocations: ArraySlice<ShadowGraph.Location>)
			...
			/// - Parameter childLocations: A list of absolute locations of children of the parent component that are to be returned or traversed after processing the nested iterator.
			indirect case deep(AsyncIterator, childLocations: ArraySlice<ShadowGraph.Location>)
		...
		mutating func next() async throws -> Child? {
			switch state {
				...
				state = .shallow(
					childLocations: try await Parent.Subject.makeShadow(graph: graph, location: location).childLocations[...]
				)
				return try await next()
				
				case .shallow(childLocations: var childLocations):
				guard let childLocation = childLocations.popFirst() else { return nil }
				let childShadow = try await graph.shadow(at: childLocation)
				let childComponent = try await childShadow.subject
				if childComponent is any FoundationalComponent {
