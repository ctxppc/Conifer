...
struct ShadowChildren<Parent : Shadow, Child> : AsyncSequence {	// TODO: Constrain `Child` to `Shadow` when `Shadow` conforms to `any Shadow`.
		...
		mutating func next() async throws -> Child? {
			switch state {
				...
				if child is any FoundationalComponent {
					...
					let childShadow = type(of: child).makeUntypedShadow(graph: graph, location: childLocation)
